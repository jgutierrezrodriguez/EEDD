...
EcoCityMoto::EcoCityMoto(vector<Moto> &vectorMotos, int tama) : vectorMotos(vectorMotos), arbolCliente(tama) {
}
...
Cliente* EcoCityMoto::BuscarCliente(string DNI) {
   ...
   char c[DNI.size() + 1];
   strcpy(c, DNI.c_str());
    arbolCliente.buscar(djb2(c),DNI,cli);
...
bool EcoCityMoto::encuentracliente(string dni) {
    ...
    char c[dni.size() + 1];
    strcpy(c, dni.c_str());
    return arbolCliente.buscar(djb2(c),dni,cli);
}
...
void EcoCityMoto::nuevoCliente(Cliente cli) {
    ...
    arbolCliente.insertar(djb2(c),cli.getDni(),cli);
}
