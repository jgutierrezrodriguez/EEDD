...
Cliente EcoCityMoto::BuscarCliente(string DNI) {
    ...
    arbolCliente.buscaIt(aux,devolver);
    
    cout<<"cliente: "<<devolver.getDni();
...
Moto EcoCityMoto::MotoCercana(UTM ubicacion) {
    ...
    for (int i = 0; i < vectorMotos.tam(); i++) {
        ...
        distancia = sqrt((ubicacion.latitud - ubicacion.longitud)*(aux.latitud - aux.longitud)
                + (aux.latitud - aux.longitud)*(ubicacion.latitud - ubicacion.longitud));
