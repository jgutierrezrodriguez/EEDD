...
int main(int argc, char** argv) {
    ...
    ListaDobleEnlazada<int> t;
    int n = 1;
    
    
    vectorClientes[1].creaitinerarios(nRand, idUlt, min,max);
    vectorClientes[10].creaitinerarios(nRand, idUlt, min,max);
    
    ListaDobleEnlazada<Itinerario> lista1(vectorClientes[1].getListaItinerario());
    ListaDobleEnlazada<Itinerario> lista2(vectorClientes[10].getListaItinerario());
    
    ListaDobleEnlazada<Itinerario>::Iterador it;
    
    it= lista1.IteradorIterador();
    
    while(!it.fin()){
        it.getNodo()->getDato().mostrar();
        it.siguiente();
    }
    
     it= lista2.IteradorIterador();
