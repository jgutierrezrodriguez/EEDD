...
PuntoRecarga* Moto::GetRecargando() const {
    return recargando;
}

void Moto::SetRecargando(PuntoRecarga* recargando) {
    this->recargando = recargando;
}

void Moto::recargar(PuntoRecarga* p) {
    recargando = p;
    darAviso();
    cout << "La moto " << id << " se esta recargando en el punto " << recargando->GetId() << "." << endl;
    float porce=100;
    SetPorcentajeBateria(porce);

}

void Moto::terminarRecarga() {
    cout << "La moto " << id << " ha terminado su recarga en el punto " << recargando->GetId() << "." << endl;
    recargando = nullptr;
}
