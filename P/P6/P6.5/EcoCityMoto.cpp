 ...
 * Author: jgr97
 * 
 * Created on 25 de octubre de 2019, 18:4
 */

#include "EcoCityMoto.h"

EcoCityMoto::EcoCityMoto(int tama,float aXMin, float aYMin, float aXMax, float aYMax, int nDivX, int nDivY) : idUltimo(0), arbolCliente(tama),
mallaPRecargas(aXMin, aYMin, aXMax,  aYMax,  nDivX,  nDivY){
}
...
vector<Moto*> EcoCityMoto::localizaMotosSinBateria() {
    ...
    for (int i = 0; i < vectorMotos.size(); i++) {
        if (vectorMotos[i].GetEstatus() == sinbateria) {
            UTM pos=vectorMotos[i].GetPosicion();
            
            if(mallaPRecargas.fueraAmbito(pos.latitud,pos.longitud)){
...
void EcoCityMoto::GuardarClientes(string nombreArchivo) {
    ...
    int i = 0;
    Cliente cli;
    while (i < arbolCliente.getNumTerminos()) {
        if (arbolCliente.getEntradas()[i].GetEstatus() == ocupado) {
            cli= arbolCliente.getEntradas()[i].GetCliente();
            i++;
            ...
            list<Itinerario> aux = arbolCliente.getEntradas()[i].GetCliente().getListaItinerarios();
...
void EcoCityMoto::eliminamilclientes() {
    vector<string>::iterator it= dniClientes.begin();
    for(int i=0; i<1000; i++){
        char c[dniClientes[i].size() + 1];
        strcpy(c, dniClientes[i].c_str());
        
        arbolCliente.borrar(djb2(c), dniClientes[i]);
        
        dniClientes.erase(it);
    }
}

vector<string> EcoCityMoto::getDniClientes() const {
    return dniClientes;
}

PuntoRecarga* EcoCityMoto::puntoRecargaCercano(Cliente* cli) {
    PuntoRecarga* p;
    p = mallaPRecargas.buscarCercano(cli->getlatitud(), cli->getlongitud());
    return p;
}

void EcoCityMoto::CargarPuntosRecarga(string file) {
    ifstream fe; //Flujo de entrada
    string linea; //Cada línea tiene un clienete
    int total = 0; //Contador de líneas o clientes
    ...
    if (fe.good()) {
        ...
        while (!fe.eof()) {
            ...
            stringstream ss; //Stream que trabaja sobre buffer interno         
                ...
                //con todos los atributos leídos, se crea el cliente
                PuntoRecarga PR(id, pos);

                //vectorMotos.push_back(mot);
                mallaPRecargas.insertar(dlat, dlon, PR);
        ...
        cout << "Total de puntos de recarga en el fichero: " << total << endl;
        fe.close(); //Cerramos el flujo de entrada
    } else {
        cerr << "No se puede abrir el fichero" << endl;
    }
