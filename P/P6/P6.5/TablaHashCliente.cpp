...
void TablaHashCliente::mostrarInfo() {
    ...
    GuardarThash("estadisticas.txt");
}

void TablaHashCliente::eliminarMILclientes() {
    int cont = 0;
    int i = 0;
    while (cont < 1000) {
        if (entradas[i].estado == ocupado) {
...
void TablaHashCliente::GuardarThash(string nombreArchivo) {
    ofstream fs(nombreArchivo, std::ofstream::app);
